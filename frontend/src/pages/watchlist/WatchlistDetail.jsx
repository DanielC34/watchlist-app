...
const WatchlistDetail = () => {
  ...
  const { getWatchlist } = useWatchlistStore();
  ...
  // 2. Open modal when bin icon is clicked
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (success) {
        await getWatchlist(); // Refresh sidebar/list state
        setIsDeleting(false);
        setIsDeleteModalOpen(false);
        // Reset currentWatchlist in your store here if possible!
        toast({
  ...
  // Redirect and show toast after successful deletion
  useEffect(() => {
      ...
      <Container maxW="container.xl" py={4}>
        <Flex justify="space-between" align="center" mb={6}>
          ...
          <Flex>
            ...
            />
            {/* 2. Open confirmation modal on bin icon click */}
            <IconButton
              icon={<FaTrash />}
